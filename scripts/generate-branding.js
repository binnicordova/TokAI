...
async function run() {
    ...
    rl.question(
        "Enter input file (e.g., logo.svg or logo.png) [logo.svg]: ",
        (inputFileAnswer) => {
            const inputFile = inputFileAnswer || "logo.svg";
            rl.question(
                "Enter brand name [tokai]: ",
                async (brandNameAnswer) => {
                    const brandName = brandNameAnswer || "tokai";
                    rl.close();
                    try {
...
async function main(inputFile, brandName) {
    ...
    const svgToPngFor = (svg, filePath, width, height) =>
        svgToPng(svg, filePath, width, height, brand, {inputIsSVG, logo});
